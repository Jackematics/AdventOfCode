...
{
    public class Day11NorthEast : Direction, IDirection
    {
        public bool AdjacentOccupiedSeatExists(Seat seat, Seat[,] seatGrid)
        {
            for (int row = seat.Position.Y - 1, column = seat.Position.X + 1; row >= 0 || column < seatGrid.GetLongLength(1); row--, column++)
            {
